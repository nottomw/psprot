...
/*!
	\brief psprot configuration structure.
*/
struct pss_config_vars {
	...
	int write_to_syslog; //!< 0 if psprot should write to syslog, 1 otherwise.
	int uid, gid; //!< UID and GID to which psprot should drop it's rights.
...
/*!
	...
	\param config psprot configuration struct.
*/
void print_config_vars( const struct pss_config_vars *config );

/*!
	\brief Frees memory allocated for psprot conf struct.
	\param config psprot conf struct to free.
*/
